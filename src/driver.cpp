...
void Driver::setup()
{
  ...
  else if (private_node_.getParam("gst_pipeline", gst_pipeline) && gst_pipeline != "")
  {
    camera_->openPipeline(gst_pipeline);
  }
  ...
  // These parameters only make sense of you are using raw opencv camera
  // If we are reading a file or using the Gsreamer back end, they make no sense
  if (gst_pipeline == "" && file_path == "")
  {
